using ArgParse, DelimitedFiles, Knet, Statistics, LinearAlgebra
...
RankingLoss(model, a, P) =
    mean((model(a)*P - model(a)).^2)

FullLoss(model, s, t, in_degrees, out_degrees, P, pmi, lambda) = 
    # ProximityLoss(model, s, t, pmi) + lambda * 
    RankingLoss(model, collect(1:1001), P)
...
function train(model, source, target, in_degrees, out_degrees, P, pmi, 
    lambda, learning_rate, epochs)
    for epoch = 1:epochs
        println("epoch = ", epoch, 
            ", loss = ", FullLoss(model, source, target, in_degrees, out_degrees, P, pmi, lambda));
        loss = @diff FullLoss(model, source, target, in_degrees, out_degrees, P, pmi, lambda);
        for param in (
                    model.W, 
